...
use super::machine::*;
//use super::protos::crash_report::CrashReport;


pub fn text_report(report: &CrashReport) -> String {
...

    {
        ...
        let incident_identifier = if report.get_report_info().has_uuid() {
            if let Ok(uuid) = Uuid::from_bytes(report.get_report_info().get_uuid()) {
                uuid
            } else {
                Uuid::new_v4()
            }
    ...
    if report.has_exception() && report.get_exception().get_frames().len() > 0 {
        ...
        for idx in 0..frames.len() {
            ...
            writeln!(text, "{}", format_stack_frame(&frame, idx, report, lp64)).unwrap();
        }
        writeln!(text).unwrap();
    }
    ...
    for image in images {
        ...
        let uuid = if image.has_uuid() {
            if let Ok(uuid) = Uuid::from_bytes(image.get_uuid()) {
                format!("{}", uuid)
            } else {
                String::from("???")
            }
...
fn format_stack_frame(frame: &CrashReport_Thread_StackFrame, idx: usize, report: &CrashReport, lp64: bool) -> String {
    ...
    let symbol_string = if frame.has_symbol() {
        ...
        format!("{} + {}", symbol_name, sym_offset)
    } else {
        format!("{:#x} + {}", base_address, pc_offset)
    };
